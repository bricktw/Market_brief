...
┌─ src/
│  ├─ sources/
│  │   ├─ finnhub.ts        # earnings, IPO, US news, US economic calendar
...
- **Finnhub free tier:** `/index/constituents` (S&P 500 / NDX membership) returns **403** on free tier — must source ticker universes from static files, not the API. `/calendar/earnings`, `/calendar/ipo`, `/calendar/economic`, and `/news` all work on free tier (verified 2026-05-18). Two gotchas: (1) `/calendar/economic` is **global** (EU/JP/KR/GB/AU/US/...), NOT US-only — `filterEconomic3Star()` only filters by impact, so non-US high-impact events still pass; narrow by `country` in the M4 renderer if desired. (2) `/news` is a **latest feed with no from/to** — it returns current news regardless of `--date`. Correct for same-day morning/evening briefs; means historical `--date` runs show today's news, not that date's. `impact` on the economic calendar is the string enum `low|medium|high` (no numeric importance on free tier).
