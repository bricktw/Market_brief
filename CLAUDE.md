...
- **M6 trigger architecture (2026-05-25, do not relitigate):** GH Actions cron is the **primary** trigger (`.github/workflows/scheduled-brief.yml`, two `schedule:` entries); the slash command (`.claude/commands/market-brief.md`) is the **manual escape hatch** for one-off / catch-up runs. Windows Task Scheduler is intentionally NOT wired (unlike Email_organizor, which needs the local machine for its Gmail MCP). The cron workflow commits the new brief back to `main` with the `github-actions[bot]` identity; `deploy-pages.yml` then publishes it. Two workflows, one boundary: scheduled-brief generates, deploy-pages publishes. Do not merge them; the separation lets manual `workflow_dispatch` on either work independently.
- **⚠️ GITHUB_TOKEN push does NOT trigger deploy-pages — MATERIALIZED 2026-07-25 (do not relitigate the fix):** the original wiring assumed `deploy-pages.yml`'s `push` + `paths: docs/**` filter would auto-publish the bot's brief commits. **It does not.** GitHub deliberately blocks a push made with the default `GITHUB_TOKEN` from triggering another workflow (recursion guard). Result: scheduled-brief kept committing briefs fine, but the **site silently froze** at the last human/`workflow_dispatch` deploy — stuck 2026-06-19 → 2026-07-25 (~51 brief commits published to the repo, none to the site) until noticed. **Fix (Option B): `deploy-pages.yml` now also triggers via `workflow_run: workflows: ["Scheduled brief"] types: [completed]`, with a build-job `if` gating on `conclusion == 'success'`.** This keeps the two-workflow boundary (chained by completion event, not merged). The stale `push`/`paths` trigger is retained for human/local pushes, which DO trigger workflows. Do NOT "simplify" by deleting `workflow_run` — that reintroduces the freeze. Rejected alternatives: PAT-based push (extra secret to maintain), a second cron on deploy-pages (redundant + timing-coupled). Diagnose a frozen site with `gh run list --workflow=deploy-pages.yml` (last run date) vs the newest `docs/briefs/` file.
